...
### Tool Outputs
- **Output Network (3D)**: The output 3D pedestrian network for further analysis. The network has the following new fields. Some fields only appear if the relevant velocity equations are checked in the dialog box:
  ...
  - ```TF_MIN_OP```: Walk time in minutes to traverse the line segment in the From-To direction, based on the 3D length of the line and an assumed walking velocity based on the slope of the line segment derived from Tobler's Hiking Function with off-path adjustment ((```End_Z```-```Start_Z```)/2D length of the line).
  ...
  - ```TF_MIN_Urb```: Walk time in minutes to traverse the line segment in the From-To direction, based on the 3D length of the line and an assumed walking velocity based on the slope of the line segment derived from the modified "Urban Adjustment" Tobler's Hiking Function ((```End_Z```-```Start_Z```)/2D length of the line).
  ...
  - ```TF_MIN_MP```: Walk time in minutes to traverse the line segment in the From-To direction, based on the 3D length of the line and an assumed walking velocity based on the slope of the line segment derived from Márquez-Pérez, Vallejo-Villalta, and Álvarez-Francoso (2017) ((```End_Z```-```Start_Z```)/2D length of the line).
  ...
  - ```TF_MIN_ICF```: Walk time in minutes to traverse the line segment in the From-To direction, based on the 3D length of the line and an assumed walking velocity based on the slope of the line segment derived from Irmischer and Clarke (2018) on-path female hiker ((```End_Z```-```Start_Z```)/2D length of the line).
  ...
  - ```TF_MIN_ICM```: Walk time in minutes to traverse the line segment in the From-To direction, based on the 3D length of the line and an assumed walking velocity based on the slope of the line segment derived from Irmischer and Clarke (2018) on-path male hiker ((```End_Z```-```Start_Z```)/2D length of the line).
